...
LOWER <- 5 # lower bound for range of topic numbers to check
UPPER <- 15 # upper bound for range of topic numbers to check
```
...
x_lda_tuning <- function(tidy_tweets_in) {
  ...
  word_counts <- tidy_tweets_in %>%
    ...
    count(id, word, sort = TRUE) %>% # count how often each org uses each word
    ungroup()
